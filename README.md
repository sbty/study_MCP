...
- [x] **2.1 Resources (リソース) の実装**
  - [x] 静的なデータやファイル内容をクライアントに提供する機能の実装
  - [x] `resources/list` と `resources/read` ハンドラーの実装
  - [x] Inspectorでのリソース読み込みテスト
- [x] **2.2 Prompts (プロンプト) の実装**
  - [x] LLMのコンテキストとして再利用可能なプロンプトテンプレートの実装
  - [x] `prompts/list` と `prompts/get` ハンドラーの実装
  - [x] 引数（Arguments）を受け取る動的プロンプトの作成
- [x] **2.3 Tools (ツール) の実装**
  - [x] LLMがアクションを実行するためのツールの定義と実装（例: 外部APIからのデータ取得や計算処理）
  - [x] `tools/list` と `tools/call` ハンドラーの実装
  - [x] JSON Schemaを用いたツールの入力引数定義

### フェーズ 3: MCPクライアント（Claude Desktop）との連携
作成したサーバーを実際のLLMクライアントに接続してテストします。

- [x] **3.1 Claude Desktopの設定**
  - [x] Claude Desktopアプリのインストール
  - [x] `claude_desktop_config.json` に自作したMCPサーバーを登録
- [x] **3.2 統合テスト**
  - [x] Claude Desktopのチャット画面から、実装したツールやリソースを呼び出せるか確認する
  - [x] エラー時のログ確認方法の習得
...
- [x] **4.1 アイデアの選定と設計**
  - [x] 連携したい外部APIやデータソースを決める（例: GitHub API連携、SQLiteデータベース連携など）
- [x] **4.2 オリジナルサーバーの実装**
  - [x] 選定したアイデアをもとに、必要なResources, Prompts, Toolsを実装する
- [x] **4.3 デバッグとリファクタリング**
  - [x] 実用的なエラーハンドリングの追加
  - [x] コードのモジュール化・リファクタリング
